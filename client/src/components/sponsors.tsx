...
let partnerScale = 0.75;
let partnerWidth = 200;
                        ...
                        <div className={`${tier} flex flex-wrap justify-center gap-6`} key={tier}>
                            {sponsorList.map((sponsor, index) => (
                                <div key={index} className="sponsor biggest mb-5">
                                    ...
                                    style={{
                                        width: ((tier == "partners")? `${partnerWidth}px` : `${Math.round(tierWidths[tier as Tier] * 1.1)}px`),
                                        height: ((tier == "partners")? `${Math.round(partnerWidth * partnerScale)}px` : `${Math.round(tierWidths[tier as Tier] * 1.3)}px`),
                                    }}  
                                    className={`relative flex flex-col items-center justify-center border-b-0 m-[10px] mx-[5px]`}>
                                        <img src={`/assets/sponsors/${sponsor.image}`} 
                                            style={{
                                                width: ((tier == "partners")? `${Math.round(partnerWidth * partnerScale)}px` : `${Math.round((tierWidths[tier as Tier] - (tierWidths[tier as Tier] * 0.17)) * sponsorScale)}px`),
                                                maxHeight: ((tier == "partners")? `${Math.round(partnerWidth * partnerScale)}px` : `${Math.round((tierWidths[tier as Tier] - (tierWidths[tier as Tier] * 0.17)) * sponsorScale)}px`)
                                            }} 
                                            className="relative z-40"/>
                                        <img src={`/assets/sponsors/tiles/${tier}.svg`} 
                                            style={{
