...
function GuildsText({innerText, className, ...props}: { innerText: string } & React.ComponentProps<"p">) {
 return (
   <p className={cn(spaceGrotesk.className, "py-5 text-center text-2xl", className)}>{innerText}</p>
 )
}
  ...
  return (
    <div className={cn("flex flex-col content-center items-start justify-center", className)} {...props}>
      <div className="flex-row justify-center items-center self-center container">
        ...
        <GuildsText innerText={"Once again, we are bringing back guilds! Think of them like hackathon houses where you’ll take on challenges as a team and earn rewards in addition to your group project."}/>
        <GuildsText innerText={"This year, we have transitioned from four to three guilds, more closely following the layout of our venue. The three floors will be home to the following guilds:"}/>
      </div>
