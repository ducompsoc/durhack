...
const hasObjectDefaultExport = (
  maybeModule: unknown,
): maybeModule is { default: Record<string, unknown> } => {
  ...
  return {
    ...
    read: async () => {
      try {
        ...
        if (!hasObjectDefaultExport(module))
          throw new Error(`Value imported from ${path} is not a module!`);
