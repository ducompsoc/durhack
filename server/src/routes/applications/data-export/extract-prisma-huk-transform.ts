...
import type { HUKDataExportBase } from "./data-export-handlers"
...
export class ExtractPrismaHUKTransform extends stream.Transform {
    ...
    this._transformAsync(chunk)
      .then((HUKPrismaData) => {
        HUKPrismaData.forEach((field) => {
          this.push(
            `${field.firstNames},${field.lastNames},${field.email},${field.phone},${field.university ?? ""},${field.graduationYear ?? ""},${field.ethnicity ?? ""},${field.gender},${field.isCheckedIn}\r`,
          )
        })
        callback(null)
      })
  ...
  async _transformAsync(chunk: HUKDataExportBase[]): Promise<HUKDataExportBase[]> {
    const HUKPrismaData = await Promise.all(
