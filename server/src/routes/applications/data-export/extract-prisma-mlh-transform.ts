...
import { MLHDataExportBase } from './data-export-handlers'
...
_transform(chunk: MLHDataExportBase[], encoding: never, callback: stream.TransformCallback): void {
    ...
    .then((MLHPrismaData) => {
      MLHPrismaData.forEach(field => {
        this.push(`${field.firstNames},${field.lastNames},${field.email},${field.phone},${field.age ?? ''},${field.university ?? ''},${field.countryOfResidence ?? ''},${field.levelOfStudy ?? ''},${field.isCheckedIn}\r`)
      })
      callback(null)
    })
...
async _transformAsync(chunk: MLHDataExportBase[]): Promise<MLHDataExportBase[]> {
    const MLHPrismaData = await Promise.all(chunk.map(async (item) => {
