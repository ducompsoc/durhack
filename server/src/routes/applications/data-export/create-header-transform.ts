...
export class CreateMLHHeaderTransform extends stream.Transform {
  ...
  _transform(chunk: string, encoding: never, callback: stream.TransformCallback): void {
    if (!this.headersAdded) {
      ...
      callback(
        null,
        "First names,Last names,Email,Phone number,Age,School,Country of residence,Current level of study,Attendance\r" +
          chunk,
      )
...
export class CreateHUKHeaderTransform extends stream.Transform {
  ...
  _transform(chunk: string, encoding: never, callback: stream.TransformCallback): void {
    if (!this.headersAdded) {
      ...
      callback(
        null,
        "First names,Last names,Email,Phone number,Academic institution,Graduation year,Ethnicity,Gender,Attendance,\r" +
          chunk,
      )
    } else callback(null, chunk)
  }
